...
import 'package:runtod_app/model/Request/RiderUpdateProfile.dart';
...
class _RiderprofileState extends State<Riderprofile> {
  ...
  String displayLicensePlate = "ทะเบียนรถ";
  ...
  void initState() {
    ...
    loadDataUser = fetchUserData().then((user) {
      ...
      licensePlateCtl.text = user.licensePlate ?? '';
    ...
    licensePlateCtl.addListener(updateDisplayNames);
  ...
  void updateDisplayNames() {
    setState(() {
      ...
      displayLicensePlate =
          licensePlateCtl.text.isEmpty ? "ทะเบียนรถ" : licensePlateCtl.text;
  ...
  Future<void> _Save() async {
    ...
    try {
      ...
      if (usernameCtl.text.trim().isEmpty ||
          ...
          licensePlateCtl.text.trim().isEmpty ||
      ...
      var data = RiderUpdateProfilePutRequest(
        uid: uid,
        username: usernameCtl.text.trim(),
        fullname: fullnameCtl.text.trim(),
        email: emailCtl.text.trim(),
        phone: phoneCtl.text.trim(),
        license_plate: licensePlateCtl.text.trim(),
      ...
      var response = await http.put(
        Uri.parse('$API_ENDPOINT/rider/update/profile'),
        headers: {"Content-Type": "application/json; charset=utf-8"},
